...
class VectorEmbedding(SQLModel, table=True):
    """Vector database table storing embeddings of workspace resources for RAG."""
    __tablename__ = "vector_embeddings"
    
    id: Optional[int] = Field(default=None, primary_key=True)
    organization_id: Optional[int] = Field(default=None, index=True)
    content_id: str = Field(index=True)  # e.g., "task:12", "user:5"
    content_type: str = Field(index=True)  # e.g., "task", "user", "leave_request"
    text_content: str = Field(sa_column=Column(Text, nullable=False))
    embedding_json: str = Field(sa_column=Column(Text, nullable=False))  # JSON representation of List[float]
    created_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))
    updated_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))
...
class WorkBotConversation(SQLModel, table=True):
    """WorkBot Chat Conversation Session."""
    __tablename__ = "workbot_conversations"
...
class WorkBotMessage(SQLModel, table=True):
    """WorkBot Chat Message."""
    __tablename__ = "workbot_messages"
    
    id: Optional[int] = Field(default=None, primary_key=True)
    conversation_id: int = Field(foreign_key="workbot_conversations.id", index=True)
    sender_role: str = Field(max_length=20)  # 'user' or 'assistant'
    content: str = Field(sa_column=Column(Text, nullable=False))
    intent: Optional[str] = Field(default=None, max_length=50)
    parameters_json: Optional[str] = Field(default=None, sa_column=Column(Text))
    navigation_url: Optional[str] = Field(default=None, max_length=255)
    created_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))


class WorkBotApiCall(SQLModel, table=True):
    """Audit log of API calls triggered by WorkBot."""
    __tablename__ = "workbot_api_calls"
    
    id: Optional[int] = Field(default=None, primary_key=True)
    conversation_id: Optional[int] = Field(default=None, foreign_key="workbot_conversations.id", index=True)
    user_id: int = Field(foreign_key="users.id", index=True)
...
class WorkBotQACache(SQLModel, table=True):
    """Q&A Semantic Cache for WorkBot."""
    __tablename__ = "workbot_qa_cache"
    ...
    parameters_json: Optional[str] = Field(default=None, sa_column=Column(Text))
    navigation_url: Optional[str] = Field(default=None, max_length=255)
    suggested_actions_json: Optional[str] = Field(default=None, sa_column=Column(Text))
    embedding_json: Optional[str] = Field(default=None, sa_column=Column(Text))  # List[float] JSON
    created_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))
