...
export default function MyDayPage() {
  ...
  const [teamTaskSheets, setTeamTaskSheets] = useState<TaskSheetEntry[]>([]);
  const [teamStartDate, setTeamStartDate] = useState<string>(todayStr);
  const [teamEndDate, setTeamEndDate] = useState<string>(todayStr);
  ...
  const loadData = useCallback(async () => {
    setIsLoading(true);
    try {
      const [tasksRes, happyRes, projectsRes, meetingRes, teamsRes, usersRes] = await Promise.all([
        getMyTasks(),
        getMyHappySheets(30),
  ...
  const loadTeamTaskSheets = useCallback(async () => {
    if (!selectedTeamId) return;
    setTeamLoading(true);
    try {
      const result = await getTeamTaskSheets(selectedTeamId, teamStartDate, teamEndDate);
      if (result.data) {
        setTeamTaskSheets(result.data);
      } else {
        setTeamTaskSheets([]);
        toast.error(result.error || "Failed to load team task sheets");
      }
    } catch (err) {
      console.error("Failed to load team task sheets", err);
      toast.error("Failed to load team task sheets");
    } finally {
      setTeamLoading(false);
    }
  }, [selectedTeamId, teamStartDate, teamEndDate]);
  ...
  const handleExportTeamSheet = async () => {
    if (!selectedTeamId) {
    ...
    try {
      const response = await exportTeamTaskSheets(selectedTeamId, teamStartDate, teamEndDate);
      if (response.data) {
        ...
        link.href = url;
        link.setAttribute("download", `team-task-sheet-${selectedTeamId}-${teamStartDate}-to-${teamEndDate}.csv`);
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        window.URL.revokeObjectURL(url);
      } else {
        toast.error(response.error || "Failed to export team task sheet");
      }
    } catch (err) {
      console.error("Export failed", err);
      toast.error("Failed to export team task sheet");
    } finally {
              ...
              <article className={`${overviewCardBase} p-5`}>
                ...
                <div className="relative space-y-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-[0.25em] text-muted-foreground">Lighthouse</p>
                      <h3 className="mt-2 text-lg font-semibold text-foreground">My Team</h3>
                    </div>
                    <button onClick={() => router.push("/my-space/task-sheet")} className="inline-flex items-center gap-1 rounded-lg border border-border/60 bg-background/60 px-2.5 py-1.5 text-xs font-semibold text-foreground hover:bg-background">
                      Manage <ArrowRight className="h-3.5 w-3.5" />
                    </button>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div className="rounded-xl border border-border/60 bg-background/40 p-3 text-center">
                      <p className="text-[11px] uppercase tracking-[0.2em] text-muted-foreground">Teams</p>
                      <p className="mt-1 text-xl font-bold text-sky-700">{teams.length}</p>
                    </div>
                    <div className="rounded-xl border border-border/60 bg-background/40 p-3 text-center">
                      <p className="text-[11px] uppercase tracking-[0.2em] text-muted-foreground">Team Members</p>
                      <p className="mt-1 text-xl font-bold text-violet-700">{selectedTeam ? selectedTeam.member_ids.length + 1 : 0}</p>
                    </div>
                  </div>

                  <div className="space-y-4">
                    <div className="space-y-2">
                      <label className="text-[11px] uppercase tracking-[0.2em] text-muted-foreground">Select Team</label>
                      <select
                        value={selectedTeamId ?? ""}
                        onChange={(event) => setSelectedTeamId(Number(event.target.value) || null)}
                        className="w-full rounded-xl border border-border/60 bg-background/70 px-3 py-2 text-sm text-foreground outline-none transition hover:border-primary"
                      >
                    ...
                    {selectedTeam ? (
                      <div className="rounded-xl border border-border/60 bg-background/40 p-4 space-y-3">
                        <div className="flex items-center gap-2 text-xs text-muted-foreground uppercase tracking-[0.2em]">
                          <Users size={14} /> Team Lead: <span className="font-semibold text-foreground">{selectedTeam.lead_name || "Unknown"}</span>
                        </div>
                        <div className="text-sm text-foreground">
                          <p className="font-semibold">Project</p>
                          <p className="text-sm text-muted-foreground">{selectedTeam.project_name || "No project assigned"}</p>
                        </div>
                          ...
                          {selectedTeam.member_ids.length > 0 ? (
                            <ul className="mt-2 space-y-1 text-sm text-muted-foreground">
                              {selectedTeam.member_ids.map((memberId) => {
                                const member = teamUsers.find((user) => user.id === memberId);
                                return (
                                  <li key={memberId}>{member ? member.name : `Member ${memberId}`}</li>
                                );
                              })}
                            </ul>
                          ) : (
                            <p className="text-xs text-muted-foreground">No additional members added.</p>
                          )}
                        </div>
                        <div className="grid gap-2 sm:grid-cols-2">
                          <input
                            ...
                            className="rounded-xl border border-border/60 bg-background/70 px-3 py-2 text-sm text-foreground outline-none"
                          />
                          <input
                            type="date"
                            value={teamEndDate}
                        ...
                        <div className="flex flex-wrap gap-2">
                          <button
                            onClick={loadTeamTaskSheets}
                            className="rounded-xl border border-border/60 bg-background/70 px-4 py-2 text-xs font-semibold text-foreground hover:bg-background"
                            disabled={!selectedTeamId || teamLoading}
                          >
                            {teamLoading ? "Refreshing..." : "Refresh Team Sheet"}
                          </button>
                          <button
                            onClick={handleExportTeamSheet}
                            className="rounded-xl bg-primary px-4 py-2 text-xs font-semibold text-white transition hover:bg-primary/90 disabled:cursor-not-allowed disabled:bg-primary/50"
                            disabled={!selectedTeamId || downloadLoading}
                          >
                            {downloadLoading ? "Exporting..." : "Download CSV"}
                          </button>
                        </div>
                        <p className="text-xs text-muted-foreground">
                          {teamTaskSheets.length > 0 ? `${teamTaskSheets.length} entries loaded for this range.` : "Use Refresh to load the current date range."}
                        </p>
                      </div>
                    ) : (
                      <div className="rounded-xl border border-dashed border-border/60 bg-background/40 p-4 text-sm text-muted-foreground">
                        Choose or create a team to review Lighthouse task sheets by date range.
                      </div>
                    )}

                    <div className="rounded-xl border border-border/60 bg-background/40 p-4 space-y-3">
                      <p className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">Create New Team</p>
                      <input
                        value={teamName}
                        onChange={(event) => setTeamName(event.target.value)}
                        placeholder="Team name"
                        className="w-full rounded-xl border border-border/60 bg-background/70 px-3 py-2 text-sm text-foreground outline-none"
                      />
                      <input
                        value={projectName}
                        ...
                        className="w-full rounded-xl border border-border/60 bg-background/70 px-3 py-2 text-sm text-foreground outline-none"
                      />
                      <label className="text-[11px] uppercase tracking-[0.2em] text-muted-foreground">Team members</label>
                      <select
                        multiple
                        value={selectedMemberIds.map(String)}
                        onChange={(event) => setSelectedMemberIds(Array.from(event.target.selectedOptions, (opt) => Number(opt.value)))}
                        className="h-32 w-full rounded-xl border border-border/60 bg-background/70 px-3 py-2 text-sm text-foreground outline-none"
                      >
                        {teamUsers.map((userOption) => (
                      ...
                      <button
                        ...
                        className="w-full rounded-xl bg-secondary px-4 py-2 text-xs font-semibold text-foreground transition hover:bg-secondary/90"
                      >
                        Create Team
                      </button>
                    </div>
