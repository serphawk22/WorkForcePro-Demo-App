...
export default function VisionaryCanvasPage() {
  ...
  return (
    <MySpaceShell>
      <div className="pb-16">
        <div className="mb-6">
          <p className="text-xs font-bold tracking-[0.2em] uppercase mb-1" style={{ color: "#854F6C" }}>VISIONARY CANVAS</p>
          <h2 className="text-3xl font-extrabold" style={{ color: "#2B124C" }}>Dream Scope</h2>
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* My Dream Project */}
          <div className="rounded-2xl p-6 shadow-sm" style={{ background: "#FBE4D8", border: "1px solid #DFB6B2" }}>
            <div className="flex items-center gap-2 mb-5">
              <Rocket size={18} style={{ color: "#522B5B" }} />
              <h3 className="font-bold text-sm" style={{ color: "#522B5B" }}>My Dream Project</h3>
            </div>
            <p className="text-sm mb-4" style={{ color: "#854F6C" }}>What is that one &quot;moonshot&quot; project you dream of building? Capture it here.</p>
            <form onSubmit={handleSubmit} className="space-y-4">
              <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Dream project is to..." rows={5} className="w-full p-3 rounded-xl text-sm focus:outline-none resize-y" style={{ background: "rgba(255,255,255,0.8)", border: "1px solid #DFB6B2" }} />
          ...
          <div>
            <div className="flex items-center gap-2 mb-4">
              <Users size={16} style={{ color: "#522B5B" }} />
              <h3 className="font-bold text-sm" style={{ color: "#522B5B" }}>Team Aspirations</h3>
            </div>
            {isLoading ? (
              ...
              <div className="rounded-xl p-8 text-center text-sm" style={{ background: "hsl(5 38% 79% / 0.15)", color: "#854F6C", border: "1px dashed #DFB6B2" }}>No team aspirations yet. Be the first to share!</div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {uniqueByUser.map((proj) => (
                  <div key={proj.id} className="rounded-xl p-4 shadow-sm" style={{ background: "rgba(255,255,255,0.85)", borderLeft: `4px solid ${colorFor(proj.user_id)}`, border: `1px solid ${colorFor(proj.user_id)}` }}>
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="font-semibold text-sm" style={{ color: "#2B124C" }}>{proj.user_name}</span>
                    </div>
                    <p className="text-xs leading-relaxed" style={{ color: "#522B5B" }}>{proj.description}</p>
                  </div>
                ))}
              </div>
            )}
