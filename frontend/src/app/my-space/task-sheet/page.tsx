      ...
      <div className="max-w-3xl mx-auto space-y-6 pb-20">
        ...
        <div className="rounded-2xl p-6 md:p-8 shadow-sm" style={{ background: "#FBE4D8", border: "1px solid #DFB6B2" }}>
          <div className="flex items-center gap-3 mb-6">
            <Swords size={24} style={{ color: "#522B5B" }} />
            <h3 className="text-xl font-bold" style={{ color: "#2B124C" }}>Log Your Operational Victories</h3>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <label className="block text-sm font-medium mb-1" style={{ color: "#522B5B" }}>Operational Victories *</label>
              <textarea value={achievements} onChange={(e) => setAchievements(e.target.value)} placeholder="Describe your achievements, learnings, and progress today..." className="w-full min-h-[140px] p-3 rounded-lg text-sm focus:outline-none transition-all resize-y" style={{ background: "rgba(255,255,255,0.7)", border: "1px solid #DFB6B2" }} maxLength={1000} />
              <div className="mt-1 text-right text-xs font-medium" style={{ color: "#854F6C" }}>{achievements.length} / 1000 characters</div>
            </div>
            <div>
              <label className="block text-sm font-medium mb-1" style={{ color: "#522B5B" }}>Direct Repository Link</label>
              <input type="url" value={repoLink} onChange={(e) => setRepoLink(e.target.value)} placeholder="https://github.com/..." className="w-full h-10 px-3 rounded-lg text-sm focus:outline-none transition-all" style={{ background: "rgba(255,255,255,0.7)", border: "1px solid #DFB6B2" }} />
            </div>
            <div className="pt-2">
        ...
        {/* History */}
        <div className="mt-8">
          <h3 className="text-lg font-bold mb-4" style={{ color: "#2B124C" }}>Recent Mission History</h3>
          {isLoadingHistory ? (
            ...
            <div className="rounded-xl p-8 text-center text-sm" style={{ background: "hsl(5 38% 79% / 0.15)", color: "#854F6C", border: "1px dashed #DFB6B2" }}>No mission records found. Log your first victory above.</div>
          ) : (
            <div className="space-y-3">
              {history.map((entry) => (
                <div key={entry.id} className="rounded-xl p-5 shadow-sm transition-shadow hover:shadow-md" style={{ background: "rgba(255,255,255,0.7)", border: "1px solid #DFB6B2" }}>
                  <div className="flex items-center gap-1.5 font-semibold text-sm mb-3" style={{ color: "#522B5B" }}>
                    <Calendar size={14} />
                    {new Date(entry.date).toLocaleDateString("en-US", { weekday: "long", year: "numeric", month: "long", day: "numeric" })}
                  </div>
                  <p className="whitespace-pre-wrap leading-relaxed text-sm" style={{ color: "#2B124C" }}>{entry.achievements}</p>
                  {entry.repo_link && (
                    <a href={entry.repo_link} target="_blank" rel="noreferrer" className="mt-3 inline-flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-lg transition-colors hover:opacity-80" style={{ background: "hsl(5 38% 79% / 0.25)", color: "#522B5B" }}>
