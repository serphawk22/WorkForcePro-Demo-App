  ...
  return (
    <MySpaceShell>
      <div className="pb-16 space-y-6">
        <div>
          <p className="text-xs font-bold tracking-[0.2em] uppercase mb-1" style={{ color: "#D97706" }}>GROWTH MINDSET</p>
          <h2 className="text-3xl font-extrabold" style={{ color: "#2B124C" }}>Learning Canvas</h2>
        </div>

        {/* My Projects */}
        <div className="rounded-2xl p-6 shadow-sm" style={{ background: "#FBE4D8", border: "1px solid #DFB6B2" }}>
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <Folder size={18} style={{ color: "#D97706" }} />
              <h3 className="font-bold text-sm" style={{ color: "#2B124C" }}>My Projects</h3>
            </div>
          ...
          {showAddProject && (
            <form onSubmit={handleAddProject} className="flex flex-wrap gap-3 mb-4 items-end">
              <div className="flex-1 min-w-[180px]"><input type="text" value={newTitle} onChange={(e) => setNewTitle(e.target.value)} placeholder="Project title" className="w-full h-9 px-3 rounded-lg text-sm focus:outline-none" style={{ background: "rgba(255,255,255,0.8)", border: "1px solid #DFB6B2" }} /></div>
              <div className="w-32"><input type="text" value={newTag} onChange={(e) => setNewTag(e.target.value.toUpperCase())} placeholder="Tag (optional)" maxLength={20} className="w-full h-9 px-3 rounded-lg text-sm focus:outline-none" style={{ background: "rgba(255,255,255,0.8)", border: "1px solid #DFB6B2" }} /></div>
          ...
          {isLoadingProjects ? (
            ...
            <p className="text-sm text-center py-4" style={{ color: "#854F6C" }}>No personal projects yet. Click &quot;Add Project&quot; to create one.</p>
          ) : (
            <div className="flex flex-wrap gap-3">
              {projects.map((proj) => (
                <div key={proj.id} className="flex items-start gap-3 rounded-xl p-4 hover:shadow-md transition-shadow" style={{ background: "rgba(255,255,255,0.8)", border: "1px solid #DFB6B2", minWidth: "180px" }}>
                  <Folder size={20} style={{ color: "#D97706", flexShrink: 0 }} />
                  <div>
                    <p className="text-sm font-semibold" style={{ color: "#2B124C" }}>{proj.title}</p>
                    {proj.tag && <span className="text-[10px] font-bold tracking-wider px-1.5 py-0.5 rounded mt-1 inline-block" style={{ background: "hsl(5 38% 79% / 0.3)", color: "#522B5B" }}>{proj.tag}</span>}
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="rounded-2xl p-6 shadow-sm" style={{ background: "#FBE4D8", border: "1px solid #DFB6B2" }}>
            <div className="flex items-center gap-2 mb-4"><Lightbulb size={18} style={{ color: "#D97706" }} /><h3 className="font-bold text-sm" style={{ color: "#D97706" }}>My Focus</h3></div>
            <form onSubmit={handleSubmitFocus} className="space-y-4">
              <p className="text-xs" style={{ color: "#854F6C" }}>What are you reading or learning right now?</p>
              <input type="text" value={focus} onChange={(e) => setFocus(e.target.value)} placeholder="Learning code igniter..." className="w-full h-10 px-3 rounded-lg text-sm focus:outline-none" style={{ background: "rgba(255,255,255,0.8)", border: "1px solid #DFB6B2" }} />
          ...
          <div>
            <div className="flex items-center gap-2 mb-4"><Users size={16} style={{ color: "#D97706" }} /><h3 className="font-bold text-sm" style={{ color: "#D97706" }}>Team Growth</h3></div>
            {isLoadingFocuses ? (
              ...
              <div className="rounded-xl p-8 text-center text-sm" style={{ background: "hsl(5 38% 79% / 0.15)", color: "#854F6C", border: "1px dashed #DFB6B2" }}>No team learning updates yet.</div>
            ) : (
              <div className="space-y-2.5">
                {uniqueByUser.map((f) => (
                  <div key={f.id} className="flex items-center gap-3 rounded-xl p-3.5 shadow-sm" style={{ background: "rgba(255,255,255,0.85)", border: "1px solid #DFB6B2" }}>
                    ...
                    <p className="text-sm" style={{ color: "#2B124C" }}><span className="font-semibold">{f.user_name}</span>{" "}<span style={{ color: "#854F6C" }}>is learning:</span>{" "}{f.focus}</p>
                  </div>
                ))}
              </div>
            )}
